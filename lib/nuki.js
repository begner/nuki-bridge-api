...
/**
 * This function returns the lastKnownState-object of the Nuki.
 *
 * @returns {Promise<lastKnownState>}
 */
Nuki.prototype.lastKnownState = function lastKnownState () {
    var self = this;
    return this
        ._request('list')
        .then(function processList (response) {
            for (const deviceInfo of response) {
                if (typeof deviceInfo !== 'object' || deviceInfo == null) {
                    continue;
                }

                if (!deviceInfo.hasOwnProperty('deviceType')) {
                    continue;
                }
                ...
                if (!deviceInfo.hasOwnProperty('nukiId')) {
                    continue;
                }

                if (String(deviceInfo.nukiId) !== String(self.nukiId)) {
                    continue;
                }

                if (!deviceInfo.hasOwnProperty('lastKnownState')) {
                    continue;
                }

                return deviceInfo.lastKnownState;
            }
        });
};
